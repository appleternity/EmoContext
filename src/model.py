...
class ConversationCNNLSTM(tf.keras.Model):
    def __init__(self, **kwargs):
        print(kwargs)
        super(ConversationCNNLSTM, self).__init__()
    ...
    def call(self, text_1, text_2, text_3):
        ...
        # cnn
        embedding_1 = self.encode_with_cnn(embedding_1)
        embedding_2 = self.encode_with_cnn(embedding_2)
        embedding_3 = self.encode_with_cnn(embedding_3)
    ...
    def encode(self, text):
        ...
        embedding = self.encode_with_cnn(embedding)
        init_state = self.cell.zero_state(text.shape[0], tf.float32)
        rnn_output, state = tf.nn.dynamic_rnn(self.dropout_cell, embedding, initial_state=init_state)
        output = self.output_dense_other_1(rnn_output[:, -1, :])
